...
#zmiana tabeli i avg (att, def) przy wygranej burzy 4:2 z sokolem
#kolejna zmiana przy wygranej burzy 4:2 z moto jelczem
...
current_stats = {
    ...
    'Sokół II Marcinkowice': {'pts': 19, 'att': 3.42, 'def': 2.92},
    'Burza Chwalibożyce': {'pts': 27, 'att': 3.92, 'def': 3.00},
    ...
    'Moto Jelcz II Oława': {'pts': 19, 'att': 4.42, 'def': 2.25},
...
fixtures = [
    ...
    ('Czarni Sobocisko', 'Lotos Gaj Oławski'),
    ...
    # Kolejka 18
    ('MKS Siechnice', 'Lotos Gaj Oławski'),
...
def simulate_match(home, away):
    # Model Poissona (bonus 24% dla gospodarza wg. statystyk z 1 polowy sezonu)
    h_lambda = current_stats[home]['att'] * current_stats[away]['def'] * 1.068 / 2.0
    a_lambda = current_stats[away]['att'] * current_stats[home]['def'] * 0.937/ 2.0
...
df_summary = pd.DataFrame(summary_data).sort_values('Śr. pozycja')
